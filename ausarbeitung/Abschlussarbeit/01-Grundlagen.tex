...
Das Algorithmus muss eine Trennebene (auf dem Beispieldiagramm ist das die schwarze Gerade im Mitte des Bildes) finden, die die Punkte auf dem Diagramm in zwei Klassen (rote und weiße Hälfte des Diagramms) teilt. Dabei soll diese Trennebene so ausgewählt werde, dass der Abstand zwischen beiden getrennten Klassen möglichst groß bleibt. Dazu werden für jede Klasse sogenannte Stützvektoren (Support Vectors) ausgewählt, die am nächsten zu der Hyberebene liegen sollen. Auf dem Diagramm \ref{fig:SVM-INTRO} sind die Stützvektoren weiß angekreuzt.
